function registerAdminRoutes(app, { authUser, store, sellerStore, paymentStore, withdrawalsCol, memory, listingsCol, usersCol, walletStore, inviteStore, portalAccess, stockStore, sellerProfilesCol, paymentsCol, cryptoAddressStore }) {
  ...
  app.post('/api/admin/sellers/:email/verify', async (req, res) => {
    try {
      ...
      res.json({ ok: true, profile: sellerStore.serializeSellerProfile(profile) });
    } catch (err) {
      res.status(500).json({ error: 'server' });
    }
  });
   ...
   * lib/payments/native-tron.js). Only 'tron-usdt-trc20' has an active
   * payment provider wired to consume it right now — the store itself is
   * network-agnostic so future chains reuse the same pool mechanics. */
  const SUPPORTED_CRYPTO_NETWORKS = ['tron-usdt-trc20'];
  ...
  app.post('/api/admin/crypto-addresses', async (req, res) => {
    try {
      ...
      if (!SUPPORTED_CRYPTO_NETWORKS.includes(network)) {
  ...
  app.get('/api/admin/crypto-addresses', async (req, res) => {
    try {
      if (!await requireAdmin(req, res)) return;
      if (!cryptoAddressStore) return res.status(503).json({ error: 'unavailable' });
      const stats = await Promise.all(SUPPORTED_CRYPTO_NETWORKS.map(n => cryptoAddressStore.poolStats(n)));
      res.json({ networks: stats });
    } catch (err) {
      res.status(500).json({ error: 'server' });
    }
  });
  ...
  app.get('/api/admin/config', async (req, res) => {
    try {
      ...
      res.json({
        ...
        sandbox: process.env.OXAPAY_SANDBOX !== 'false',
