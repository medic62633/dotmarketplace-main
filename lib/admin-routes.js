...
function registerAdminRoutes(app, { authUser, store, sellerStore, paymentStore, withdrawalsCol, memory, listingsCol, usersCol, walletStore, inviteStore, portalAccess, stockStore, sellerProfilesCol, paymentsCol, cryptoAddressStore }) {
  ...
  app.post('/api/admin/sellers/:email/verify', async (req, res) => {
    try {
      ...
      // Only announce a state change: re-verifying an already-verified seller
      // would otherwise re-send the welcome every time an admin clicks it.
      if (!profile.verified) notify.sellerApproved({ email, name: user?.name || profile.name });
  ...
  app.post('/api/admin/sellers/:email/reject', async (req, res) => {
    try {
      ...
      if (profile.verified) {
        notify.sellerRejected({ email, name: user?.name || profile.name, reason: String((req.body || {}).reason || '').trim() || undefined });
      }
      res.json({ ok: true });
    } catch (err) {
      res.status(500).json({ error: 'server' });
  ...
  app.post('/api/admin/withdrawals/:id/approve', async (req, res) => {
    try {
      ...
      // alreadyProcessed returns above, so reaching here means THIS call is the
      // one that marked it sent — the email cannot go out twice.
      if (result.alreadyProcessed) return res.status(400).json({ error: 'already processed' });
      const doc = result.doc || {};
      notify.withdrawalPaid({
        email: doc.sellerEmail,
        amount: doc.amount,
        address: doc.address,
        networkLabel: doc.network ? 'USDT ' + doc.network : undefined,
        txHash,
        paidAt: doc.processedAt || new Date(),
        reference: doc._id,
      });
  ...
  app.post('/api/admin/withdrawals/:id/reject', async (req, res) => {
    try {
      ...
      if (result.notFound) return res.status(404).json({ error: 'not found' });
      if (result.alreadyProcessed) return res.status(400).json({ error: 'already processed' });
      const credited = await sellerStore.creditSellerBalance(w.sellerEmail, w.amount, {
        type: 'refund',
        amt: w.amount,
      ...
      // After the credit, not before — the email states the money is back.
      notify.withdrawalRejected({
        email: w.sellerEmail,
        amount: w.amount,
        reason: String((req.body || {}).reason || '').trim() || undefined,
        rejectedAt: new Date(),
        reference: w._id,
        balance: credited && credited.profile ? credited.profile.balance : undefined,
      });
      res.json({ ok: true });
    } catch (err) {
      res.status(500).json({ error: 'server' });
  ...
  app.post('/api/admin/disputes/:dealId/resolve', async (req, res) => {
    try {
      ...
      // Read the deal before resolving: resolveEscrow returns an escrow only on
      // the refund path, so the release path would otherwise have nothing to
      // address the notification to.
      const escrow = await sellerStore.getEscrow(req.params.dealId);
      ...
      // Past every early return, so this call is the one that decided it.
      // `escrow` was read before resolving, so its status is the PRE-decision
      // one — which is what says whether this was a dispute at all.
      notify.escrowResolved({
        escrow: result.escrow || escrow,
        wasDisputed: escrow ? escrow.status === 'dispute' : false,
        outcome: result.released ? 'released' : 'refunded',
