    ...
    try {
      ...
      if (result.full && result.added === 0) {
        return res.status(409).json({
          error: 'stock_full',
          msg: `This listing already holds the maximum of ${result.limit} stock units — sell or remove some before adding more`,
        });
      }
      res.json({
        ok: true,
        ...
        // Non-zero when the paste ran past the listing's cap: say so rather
        // than reporting success for lines that were silently dropped.
