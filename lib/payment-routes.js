...
function registerPaymentRoutes(app, { authUser, paymentStore, sellerStore, walletStore, onPaymentPaid, paymentLimiter, stockStore, cryptoAddressStore }) {
  ...
  async function syncProviderPayment(record) {
    ...
    if (record.provider === 'native_tron' && record.payAddress) {
      try {
        const seen = await payments.nativeTron.checkAddressForPayment(record.payAddress, record.buyerTotal || record.amount);
        if (seen.found) {
          ...
          if (waited >= payments.nativeTron.confirmWindowMs()) {
            return markPaid(record, 'native_tron_poll', { amount: seen.amount, txId: record.nativeTxId });
          }
        }
      } catch (err) {
        console.error('native_tron status sync error:', err.message);
      }
    }
  ...
  function applyProviderDetails(record, payload) {
    ...
    } else if (record.provider === 'native_tron') {
      record.payNetwork = 'TRON';
      record.payTxNetwork = 'native_tron';
      record.payCurrency = 'USDT';
