  ...
  async function syncProviderPayment(record) {
    ...
    if (native && record.payAddress) {
      try {
        ...
        // figures and would be the wrong unit entirely for those three.
        const expected = record.payAmount ?? record.buyerTotal ?? record.amount;
        const seen = await native.checkAddressForPayment(record.payAddress, expected, record.nativeContext);
        if (seen.found) {
