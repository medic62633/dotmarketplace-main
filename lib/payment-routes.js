...
function registerPaymentRoutes(app, { authUser, paymentStore, sellerStore, walletStore, onPaymentPaid, paymentLimiter, stockStore, cryptoAddressStore }) {
  ...
  async function creditDeposit(record) {
    ...
    if (walletStore && doc.buyerEmail && creditAmount > 0) {
      ...
      // for these); label it by chain instead of defaulting to "Simulated"
      // just because it isn't OxaPay. Only the local-simulate endpoint can
      // ever reach 'paid' when neither OxaPay nor a native provider is
      // configured, so that combination is the unambiguous simulated case.
      const nativeLabel = NATIVE_NETWORK_LABELS[doc.provider];
      const lbl = doc.provider === 'oxapay' ? 'Deposit · OxaPay'
        : nativeLabel ? `Deposit · ${nativeLabel.payNetwork}`
        : 'Deposit · Simulated';
  ...
  app.post('/api/payments/escrow', payLimit, async (req, res) => {
    try {
      ...
      record = {
        ...
        // undefined for oxapay/cryptomus and for native_tron (no context needed).
  ...
  async function syncProviderPayment(record) {
    ...
    // Any "no processor" chain provider (TRON, EVM, Solana, UTXO — see
    // lib/payments/index.js's NATIVE_PROVIDERS registry) is handled
    // identically here: poll for an observed transfer, wait out that
    // provider's confirmWindowMs() once one is seen, then mark paid through
    // the exact same path oxapay uses.
    const native = payments.NATIVE_PROVIDERS[record.provider];
    if (native && record.payAddress) {
      try {
        const seen = await native.checkAddressForPayment(record.payAddress, record.buyerTotal || record.amount, record.nativeContext);
        if (seen.found) {
          if (!record.nativeFirstSeenAt) {
            record.nativeFirstSeenAt = new Date();
            record.nativeTxId = seen.txId;
            record.nativeFrom = seen.from;
            await paymentStore.save(record);
          }
          const waited = Date.now() - new Date(record.nativeFirstSeenAt).getTime();
          if (waited >= native.confirmWindowMs()) {
            return markPaid(record, record.provider + '_poll', { amount: seen.amount, txId: record.nativeTxId });
          }
        }
      } catch (err) {
  ...
  // Friendly display label for each native provider — shown as payNetwork,
  // same field oxapay populates from its own provider response.
  const NATIVE_NETWORK_LABELS = {
    native_tron: { payNetwork: 'TRON', payCurrency: 'USDT' },
    native_eth: { payNetwork: 'Ethereum', payCurrency: 'USDT' },
    native_bsc: { payNetwork: 'BSC', payCurrency: 'USDT' },
    native_sol: { payNetwork: 'Solana', payCurrency: 'SOL' },
    native_sol_usdt: { payNetwork: 'Solana', payCurrency: 'USDT' },
    native_btc: { payNetwork: 'Bitcoin', payCurrency: 'BTC' },
    native_ltc: { payNetwork: 'Litecoin', payCurrency: 'LTC' },
  };
  ...
  function applyProviderDetails(record, payload) {
    ...
    } else if (NATIVE_NETWORK_LABELS[record.provider]) {
      Object.assign(record, NATIVE_NETWORK_LABELS[record.provider]);
