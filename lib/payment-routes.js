...
function registerPaymentRoutes(app, { authUser, paymentStore, sellerStore, walletStore, onPaymentPaid, paymentLimiter, stockStore, cryptoAddressStore }) {
  ...
  async function creditDeposit(record) {
    ...
    if (walletStore && doc.buyerEmail && creditAmount > 0) {
      ...
      const credited = await walletStore.deposit(doc.buyerEmail, creditAmount, lbl);
      // Hooked here rather than at the two callers (the status poll and the
      // sandbox simulate endpoint) because claimPaid above is the single
      // atomic claim — whichever path gets here, exactly one of them does,
      // so the buyer is told once and only after the balance moved.
      notify.walletToppedUp({
        email: doc.buyerEmail,
        amount: creditAmount,
        balance: credited && credited.balance != null ? credited.balance : undefined,
        networkLabel: doc.networkLabel || (nativeLabel ? nativeLabel.payNetwork : undefined),
        txHash: doc.nativeTxId,
        creditedAt: doc.paidAt || new Date(),
        orderId: doc.orderId,
      });
    }
    return doc;
  ...
  app.post('/api/payments/escrow', payLimit, async (req, res) => {
    try {
      ...
      // A fresh invoice only — the two early returns above hand back an
      // existing pending or already-paid record without reaching here, so
      // re-opening checkout on the same order does not re-send the address.
      // confirmWindowMs is how long this provider waits after first seeing a
      // transfer before treating it as settled — the buyer's real wait, and
      // the only confirmation figure the providers actually export.
      const confirmMs = payments.NATIVE_PROVIDERS[record.provider]?.confirmWindowMs?.();
      notify.paymentInstructions({
        ...record,
        confirmMinutes: Number.isFinite(confirmMs) && confirmMs > 0 ? Math.max(1, Math.round(confirmMs / 60000)) : undefined,
      });
