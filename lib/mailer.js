...
async function sendMail({ to, subject, text, html }) {
  const c = config();
  if (!c.enabled) {
    // The subject is NOT logged: a verification email's subject line is the
    // 6-digit code itself ("123456 — your Dot Marketplace code"), so logging
    // it wrote live account-recovery codes into the server log, where they
    // outlive the code's own 10-minute expiry and reach anyone who can read
    // logs. The recipient alone is enough to see that a send was attempted.
    console.log('   mail (disabled) → ' + to);
