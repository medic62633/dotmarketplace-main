function createPaymentStore({ memory, paymentsCol }) {
  ...
  /* Atomically record that a native-chain transfer was first observed for a
   * still-pending record. Unlike a blind save() of a whole in-memory record
   * object, this only ever touches these three fields and only when the
   * record is still 'pending' with no prior sighting — so it can never
   * clobber a concurrent, legitimate transition (an admin's manual cancel,
   * or another poll's own claimPaid) that landed on the same document in
   * the tiny window between this call's read and its write. No-ops
   * (claimed: false) once the record has moved on or was already recorded;
   * the caller has nothing further to do in that case either way. */
  async function recordNativeSeen(id, { nativeFirstSeenAt, nativeTxId, nativeFrom }) {
    const patch = { nativeFirstSeenAt, nativeTxId, nativeFrom, updatedAt: new Date() };
    if (memory) {
      const doc = memory.payments.get(id);
      if (!doc || doc.status !== 'pending' || doc.nativeFirstSeenAt) return { claimed: false, doc: doc || null };
      Object.assign(doc, patch);
      memory.payments.set(id, doc);
      return { claimed: true, doc };
    }
    const res = await paymentsCol.findOneAndUpdate(
      { _id: id, status: 'pending', nativeFirstSeenAt: { $exists: false } },
      { $set: patch },
      { returnDocument: 'after' }
    );
    const doc = res && (res.value !== undefined ? res.value : res);
  ...
  return { getByOrderId, getById, save, claimPaid, claimTerminal, claimCheckout, releaseCheckout, recordNativeSeen, listRecent, listBySeller, listByBuyer };
}
