...
function applyListingImage(doc, image, dataUrl, ownerKey) {
  const oldImage = doc.image || null;
  const cleanupOld = next => {
    if (oldImage && oldImage !== next) deleteListingImageFile(oldImage, PUBLIC_DIR);
  };
  if (dataUrl) {
    const next = saveListingImage(dataUrl, PUBLIC_DIR, ownerKey);
    cleanupOld(next);
    doc.image = next;
    return;
  }
  ...
  if (typeof image === 'string') {
    if (!isListingImageUrl(image)) throw new Error('invalid image');
    doc.image = image;
    return;
  }
}
...
function registerSellerRoutes(app, { authUser, store, sellerStore, sellerEmailFromName, paymentStore, walletStore, writeLimiter, readLimiter }) {
  ...
  app.post('/api/seller/listings', writeLimit, async (req, res) => {
    try {
      ...
      if (!user) return;
      const profile = await sellerStore.getSellerProfile(user._id);
      const { cat, title, desc, price, variants: rawVariants, status, image, imageData } = req.body || {};
      ...
      try {
        if (imageData) applyListingImage(doc, undefined, imageData, user._id.replace(/[^a-z0-9]/gi, ''));
        else if (image) applyListingImage(doc, image, null);
      } catch (imgErr) {
  ...
  app.put('/api/seller/listings/:id', writeLimit, async (req, res) => {
    try {
      ...
      try {
        if (imageData) applyListingImage(doc, undefined, imageData, user._id.replace(/[^a-z0-9]/gi, ''));
        else if (Object.prototype.hasOwnProperty.call(req.body || {}, 'image')) applyListingImage(doc, image, null);
      } catch (imgErr) {
        return res.status(400).json({ error: 'image', msg: imgErr.message === 'too large' ? 'Image too large (max 4 MB)' : imgErr.message === 'storage_limit' ? 'Image storage limit reached (20 MB) — remove old images' : 'Invalid image' });
      }
      await sellerStore.saveListing(doc);
  ...
  app.delete('/api/seller/listings/:id', async (req, res) => {
    try {
      ...
      if (doc.image) {
        deleteListingImageFile(doc.image, PUBLIC_DIR);
        doc.image = null;
      }
      doc.status = 'removed';
      await sellerStore.saveListing(doc);
  ...
  app.get('/api/seller/orders', async (req, res) => {
    try {
      const user = await requireSeller(req, res);
      if (!user) return;
      let orders = paymentStore
        ? await paymentStore.listBySeller(user._id, 50)
        : [];
      if (paymentStore) {
        for (const o of orders) {
          if (o.status === 'paid' && o.provider === 'oxapay' && !o.payNetwork && o.providerPaymentId) {
            await oxapay.enrichRecordNetwork(o);
            await paymentStore.save(o);
          }
        }
      }
      res.json({
