...
function registerSellerRoutes(app, { authUser, store, sellerStore, sellerEmailFromName, paymentStore, walletStore, writeLimiter, readLimiter }) {
  ...
  app.put('/api/seller/withdraw-address', async (req, res) => {
    try {
      ...
      await sellerStore.ensureSellerProfile(user._id, user.name);
      // Targeted $set: writing the whole profile back would revert any payout
      // credit that landed between the read and the write.
      const profile = await sellerStore.updateSellerProfileFields(user._id, {
        withdrawAddress: address.slice(0, 120),
      });
  ...
  app.post('/api/seller/withdrawals', writeLimit, async (req, res) => {
    try {
      ...
      // `profile` here is the document debitSellerBalance returned, i.e. a
      // snapshot from BEFORE this line. Writing it back wholesale reverted
      // anything that landed in between — including a payout credit and its
      // ledger entry. Only the address is being changed, so only set that.
      let profile = result.profile;
      if (address) {
        profile = await sellerStore.updateSellerProfileFields(user._id, {
          withdrawAddress: address.slice(0, 120),
        }) || profile;
      }
      // Sent only after the debit AND the withdrawal record both committed —
      // the saveWithdrawal failure path above refunds and rethrows, so a
      // seller never gets "withdrawal requested" for one that does not exist.
      notify.withdrawalRequested({
        email: user._id,
        amount: withdrawal.amount,
        address: withdrawal.address,
        networkLabel: 'USDT ' + withdrawal.network,
        requestedAt: withdrawal.createdAt,
        reference: withdrawal._id,
        balance: profile ? profile.balance : undefined,
      });
      res.json({
        withdrawal: {
          id: withdrawal._id,
  ...
  app.post('/api/seller/deliver', async (req, res) => {
    try {
      ...
      // After the state change, never before: an email saying "delivered" must
      // not go out for a transition that did not happen. _setEscrowState only
      // returns an escrow when it actually claimed the 'held' -> 'delivered'
      // move, so a repeated call falls into badState above and re-sends nothing.
      notify.orderDelivered({ escrow: result.escrow, proof, sellerName: user.name });
  ...
  app.post('/api/seller/dispute', async (req, res) => {
    try {
      ...
      // Both sides, worded for their side: the seller has to respond in the
      // deal chat, the buyer needs to know their money did not move.
      notify.disputeOpened({ escrow: result.escrow, reason: String(reason).trim() });
  ...
  app.post('/api/seller/payout', writeLimit, async (req, res) => {
    try {
      ...
      // Read the escrow before the release so the emails below have the deal's
      // title and counterparties; creditSellerPayout returns only the money.
      const escrow = await sellerStore.getEscrow(dealId);
      ...
      // Only on the real release. `duplicate` returned above, so reaching here
      // means this call is the one that moved the money.
      notify.escrowReleased({
        escrow,
        gross: escrow ? escrow.amount : undefined,
        fee: result.fee,
        net: result.net,
        balance: result.profile ? result.profile.balance : undefined,
      });
