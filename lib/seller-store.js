...
function serializeListing(doc, sellerProfile) {
  ...
  return {
    ...
    // `image` stays the cover/first photo for every existing single-image
    // consumer (card thumbnails, search results, the seller listing list).
    // `images` is the full gallery — always an array, even for a listing
    // saved before multi-image support existed (falls back to the one
    // `image` it already had), so the client never has to branch on which
    // shape a given listing is in.
    image: doc.image || null,
    images: Array.isArray(doc.images) && doc.images.length ? doc.images : (doc.image ? [doc.image] : []),
  };
}

function createSellerStore({ memory, listingsCol, sellerProfilesCol, withdrawalsCol, escrowsCol, reviewsCol, mongoClient, txnSupported }) {
  ...
  /**
   * Run `fn(session)` inside a real multi-document Mongo transaction when the
   * deployment supports one (a replica set — Atlas always does; a standalone
   * mongod, as used by a plain `docker run mongo`, does not). `fn` must be
   * idempotent-safe to retry, same as every other write in this module: the
   * driver's withTransaction retries the whole callback on a transient
   * TransientTransactionError.
   *
   * Falls back to `fn(null)` — the exact single-writes-in-sequence behavior
   * this module already had — on the memory store or when the connected
   * Mongo isn't a replica set, so environments that can't support
   * transactions see zero behavior change from before this helper existed.
   */
  async function runTxn(fn) {
    if (memory || !txnSupported || !mongoClient) return fn(null);
  ...
  /* Writes the WHOLE document. Only safe when the caller owns every field it
   * is about to overwrite — i.e. creating a profile, or inside a memory-store
   * lock. For changing a field or two on an existing profile use
   * updateSellerProfileFields instead: a seller profile carries live money
   * (balance, pendingEscrow, totalEarnings, deals, ledger) that
   * creditSellerPayout and friends move with atomic $inc, and a full replace
   * built from a document read moments earlier silently reverts any of those
  ...
  /**
   * Set specific fields on a seller profile without touching anything else.
   *
   * This exists because the read-modify-replace pattern loses money. A seller
   * requesting a withdrawal, an admin verifying them, or a buyer leaving a
   * review would each read the profile, change one unrelated field, and write
   * the whole document back — reverting any payout credit ($inc on balance /
   * totalEarnings / deals plus its ledger entry) that landed in the gap. The
   * withdrawal path was the worst of them: it wrote back a snapshot taken
   * BEFORE its own debit.
   *
   * Returns the updated document (Mongo) or the in-memory object.
   */
  async function updateSellerProfileFields(email, fields) {
    const patch = { ...fields, updatedAt: new Date() };
    delete patch._id; // never let a caller rewrite the key
    if (memory) {
      const profile = await getSellerProfile(email);
      if (!profile) return null;
      Object.assign(profile, patch);
      memory.sellerProfiles.set(email, profile);
      return profile;
    }
  ...
  async function saveEscrow(rec, { session } = {}) {
    if (memory) {
      memory.escrows.set(rec._id, rec);
      return;
    }
    await escrowsCol.replaceOne({ _id: rec._id }, rec, { upsert: true, session });
  }

  /* An existing escrow under this deal id that belongs to someone else is not
   * a duplicate of THIS purchase — it's a collided/forged id. Callers must
   * treat it as an error rather than a harmless replay, or they charge one
   * buyer and leave every record on another buyer's deal. The routes reject
   * such an id before reaching here (lib/order-owner.js); this is the last
   * line, so the store can never quietly report "already held" for a hold it
   * did not actually take. */
  function isOtherBuyer(existing, buyerEmail) {
    return !!(buyerEmail && existing?.buyerEmail && existing.buyerEmail !== buyerEmail);
  }
   ...
   *
   * Returns { escrow, duplicate, mismatch } — `mismatch` means the stored
   * escrow is a different buyer's and nothing was recorded for this caller.
   */
  async function holdEscrow({ dealId, buyerEmail, sellerEmail, amount, method, title }) {
    ...
    if (memory) {
      return withLock('hold:' + dealId, async () => {
        ...
        if (existing) return { escrow: existing, duplicate: true, mismatch: isOtherBuyer(existing, buyerEmail) };
        const rec = {
          _id: dealId,
  ...
  async function creditSellerPayout({ dealId, buyerEmail }) {
    ...
    // Mongo: claim the release atomically, then credit the seller. Previously
    // these were separate un-transacted writes — a crash between the escrow
    // claim and the balance credit left the escrow terminally 'released' with
    // the seller never paid (no other code path retries a terminal escrow).
    // Wrapped in a transaction (when the deployment supports one — see
    // runTxn): either both the claim and the credit land, or neither does,
    // and a retried attempt sees the same pre-claim state it started from.
    await ensureSellerProfile(sellerEmail);
    const outcome = await runTxn(async (session) => {
    ...
    if (outcome.duplicate) return { duplicate: true };
  ...
  async function _setEscrowState(dealId, fromStates, patch, session) {
    if (memory) {
  ...
  async function _resolveEscrow({ dealId, resolution, walletStore, paymentStore, stockStore }) {
    ...
    if (resolution === 'refund') {
      // Decide whether this escrow was genuinely wallet-funded before any
      // mutation. Trust the verified payment record when present; fall back
      // to the escrow method (escrows opened before a payment record existed
      // are wallet-only here).
      let payment = null;
      ...
      // The escrow-state claim + the actual money movement (wallet credit, or
      // the pendingEscrow drawdown) are wrapped as one atomic unit — see
      // runTxn. Previously these were separate un-transacted writes: a crash
      // right after the escrow flipped to the terminal 'refunded' state, but
      // before the wallet was credited, permanently stranded the buyer's
      // money (no other code path retries a terminal escrow). Recording the
      // payment's refundStatus flag and retiring the stock unit stay OUTSIDE
      // the transaction, unchanged from before: they're non-fatal bookkeeping
      // (an operator dashboard hint, a reuse guard), not money, and the
      // original code already tolerated either failing silently — pulling
      // them into the transaction would instead let one of those secondary
      // writes fail and roll back the buyer's actual refund, which is worse.
      const result = await runTxn(async (session) => {
        const opts = session ? { session } : {};
        ...
        if (isWalletBacked && walletStore && escrow.buyerEmail) {
          // Return internal funds to the buyer wallet and release the hold so
          // the buyer isn't left with a stuck hold blocking a re-payment.
          await walletStore.deposit(escrow.buyerEmail, escrow.amount, `Refund · ${dealId}`, opts);
          await walletStore.releaseHold(escrow.buyerEmail, dealId, opts);
        } else {
          // Crypto-backed: do NOT mint internal wallet balance. The escrow
          // record itself is flagged so an operator issues the refund at the
          // provider off-platform.
          claim.escrow.refundMethod = 'crypto_external';
          await saveEscrow(claim.escrow, opts);
        }
        if (memory) {
      ...
      if (payment) {
        payment.refundStatus = isWalletBacked ? 'wallet_credited' : 'requires_provider_refund';
  ...
  async function addReview({ dealId, buyerEmail, buyerName, rating, text }) {
    ...
    await ensureSellerProfile(escrow.sellerEmail);
    const all = await listReviewsForSeller(escrow.sellerEmail);
    // Only the rating changes here. Writing the whole profile back would
    // revert a payout credit that landed between the read and the write —
    // a review is posted right after a release, so that window is real.
    const profile = await updateSellerProfileFields(escrow.sellerEmail, {
      rate: recalculateRate(all),
    });
