...
function createStockStore({ memory, stockCol }) {
  ...
  async function addStock(listingId, sellerEmail, secrets, variantId) {
    ...
    const list = (Array.isArray(secrets) ? secrets : [])
      .map(s => String(s || '').trim())
      .filter(s => s.length > 0)
      .slice(0, MAX_STOCK_PER_LISTING);
    if (!list.length) return { added: 0, available: await countAvailable(listingId, vid) };
    ...
    return { added: docs.length, available: await countAvailable(listingId, vid) };
  }
  ...
  async function retireForOrder(orderId) {
    const patch = {
      status: 'compromised',
  ...
  return { addStock, countAvailable, claimOne, getForOrder, hasAny, retireForOrder, reserveOne, finalizeClaim, releaseReservation, reveal, MAX_STOCK_PER_LISTING };
}
