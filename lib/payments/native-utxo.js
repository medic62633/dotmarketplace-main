...
function createUtxoProvider({ network, providerName, envPrefix, symbol, defaultApiBase, defaultConfirmSecs }) {
  ...
  async function createInvoice({ orderId, amount }) {
    ...
    // funded_txo_sum is the total this address has EVER received and only ever
    // grows, so this baseline is what separates this order's payment from
    // every payment the address took before it. It used to swallow an
    // explorer failure and fall back to 0, which on a recycled address (the
    // pool returns them on cancel/expiry) makes its entire lifetime funding
    // count as this order's payment — instantly paid, nothing sent.
    //
    // Failing the checkout is recoverable: claimForOrder is idempotent, so a
    // retry reuses the same address and re-reads the total.
    let priorFunded;
    try {
      priorFunded = await fundedSatoshis(claim.doc.address);
    } catch (err) {
      throw new Error(`${network}: could not read the deposit address's funding baseline — ${err.message}`);
    }
    if (!Number.isFinite(Number(priorFunded))) {
      throw new Error(`${network}: explorer returned an unusable funding baseline for the deposit address`);
    }
    return {
      provider: network,
  ...
  async function checkAddressForPayment(address, expectedAmount, context) {
    ...
    // No baseline means this order's payment can't be told apart from the
    // address's earlier funding — refuse rather than assume 0.
    const prior = Number(context?.priorFunded);
    if (!Number.isFinite(prior)) {
      throw new Error(`${network}: payment record has no priorFunded baseline — cannot safely check for payment`);
    }
    const current = await fundedSatoshis(address);
    const deltaCoin = (current - prior) / 1e8;
    if (!Number.isFinite(deltaCoin) || deltaCoin <= 0) return { found: false };
    // Quoted to 8 places by createInvoice's convertUsdToCoin call, so one
    // satoshi is the whole rounding allowance — see lib/payments/amounts.js.
    if (!coversExpected(deltaCoin, expectedAmount, 8)) return { found: false };
    return { found: true, amount: deltaCoin, txId: null, from: null };
  }
...
const nativeBtc = createUtxoProvider({
  ...
  // ~2 extra blocks' worth of wall-clock time at Bitcoin's ~10min average —
  // chain_stats already guarantees 1 real confirmation (see doc comment);
  // this is margin on top, not the primary gate, so it's a deliberately
  // modest buffer rather than the deeper wait a "wait for 6 confirmations"
  // rule of thumb would imply.
  defaultConfirmSecs: 15 * 60,
});
...
const nativeLtc = createUtxoProvider({
  ...
  // ~2 extra blocks at Litecoin's ~2.5min average — same margin rationale as
  // BTC above, scaled to LTC's faster block time.
  defaultConfirmSecs: 5 * 60,
});
