 ...
 * calling out at all — useful as a manual override/fallback. NATIVE_FX_API_BASE
 * overrides the CoinGecko base URL, the same *_API_BASE pattern every other
 * native provider in this codebase already uses — lets a test point it at a
 * deterministically-unreachable address to exercise the failure path without
 * depending on whether the environment actually has outbound internet access
 * (this sandbox doesn't; a real CI runner typically does, so "no network"
 * can't be the thing a test relies on to force this path).
 */

const COINGECKO_IDS = { BTC: 'bitcoin', LTC: 'litecoin', SOL: 'solana' };
function apiBase() {
  return (process.env.NATIVE_FX_API_BASE || 'https://api.coingecko.com/api/v3').replace(/\/$/, '');
}
...
async function fetchLivePrice(symbol) {
  ...
  const url = `${apiBase()}/simple/price?ids=${id}&vs_currencies=usd`;
 ...
 * if no usable price is available — see usdPrice(). Also throws if the
 * amount rounds to zero at that precision (a very low USD amount against a
 * very high coin price/coarse decimals): a payAmount of 0 isn't just an
 * unpayable invoice, it's a live bug in the caller's on-chain check — a
 * balance delta of literally nothing (0) satisfies "received >= 0", so
 * checkAddressForPayment would report ANY order paid with zero evidence
 * anyone sent anything. Never let that amount leave this function. */
async function convertUsdToCoin(symbol, usdAmount, decimals) {
  ...
  const rounded = Math.round(coinAmount * factor) / factor;
  if (!(rounded > 0)) {
    throw new Error(`${usdAmount} USD is too small to represent in ${symbol} at the current rate (rounds to 0 at ${decimals} decimals) — cannot create a payable invoice`);
  }
  return rounded;
}
