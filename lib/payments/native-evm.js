...
function createEvmProvider({ network, providerName, envPrefix, defaultRpcUrl, defaultContract, defaultDecimals }) {
  function rpcUrl() {
   ...
   * { found: false }. Does NOT decide the order is paid — same policy split
   * as native-tron.js: the caller applies confirmWindowMs() before trusting
   * this.
   */
  async function checkAddressForPayment(address, expectedAmount, context) {
    if (!address) return { found: false };
    const fromBlock = context?.fromBlock != null ? '0x' + Number(context.fromBlock).toString(16) : '0x0';
