...
function createWalletStore({ memory, walletsCol }) {
  ...
  async function getWallet(email) {
    ...
    return walletsCol.findOne({ _id: email });
  }
  ...
  async function save(w) {
    ...
    await walletsCol.replaceOne({ _id: w._id }, w, { upsert: true });
  }
  ...
  async function ensureWallet(email, seedBalance) {
    let w = await getWallet(email);
    if (!w) {
      w = blank(email);
  ...
  async function deposit(email, amount, lbl) {
  ...
  async function releaseHold(email, dealId) {
    if (!dealId) return null;
    if (memory) {
      const w = await getWallet(email);
