 ...
 *
 * Top-ups are funded by a real OxaPay invoice: the wallet is credited only
 * when the deposit is confirmed (webhook, provider status check, or — in
 * sandbox — a local simulate).
 */
const crypto = require('crypto');
const payments = require('./payments');
const oxapay = require('./payments/oxapay');
...
function registerWalletRoutes(app, { authUser, walletStore, sellerStore, paymentStore, creditDeposit, paymentLimiter, stockStore }) {
  ...
  // Start a wallet top-up via OxaPay. Creates a deposit invoice; the wallet is
  // credited only once the payment is confirmed (webhook / status check).
  app.post('/api/wallet/topup', payLimit, async (req, res) => {
    try {
      ...
      if (payments.provider() === 'native_tron' && payments.nativeTron.configured()) {
        record.provider = 'native_tron';
        record.method = 'native_tron';
        const invoice = await payments.nativeTron.createInvoice({ orderId, amount });
      ...
      } else if (oxapay.configured()) {
        const callbackUrl = `${payments.publicUrl()}/api/webhooks/oxapay`;
        const successUrl = `${payments.publicUrl()}/#wallet`;
        ...
        record.providerPaymentId = invoice.providerPaymentId;
        record.payUrl = invoice.payUrl;
        // In sandbox the invoice is real but can also be completed locally.
        record.sandbox = oxapay.sandboxMode();
      } else {
        // No OxaPay key on this server. Only allow local simulation when the
        // server explicitly permits it; otherwise there is no way to fund a
        // wallet and we say so instead of silently enabling free credits.
      ...
      res.json({
        ...
        configured: oxapay.configured() || payments.nativeTron.configured(),
      });
    } catch (err) {
  ...
  app.get('/api/wallet/topup/:orderId', async (req, res) => {
    try {
      ...
      // Pull live status from OxaPay so confirmation works without waiting on a
      // webhook (useful for local/dev where callbacks can't reach the server).
      if (record.status !== 'paid' && record.provider === 'oxapay' && record.providerPaymentId) {
        try {
          const info = await oxapay.getPaymentInfo(record.providerPaymentId);
          if (info && oxapay.isPaidStatus(info.status)) {
            record._providerInfo = info;
            record = await creditDeposit(record);
          }
        } catch (e) { /* fall through with current status */ }
      }
      // native_tron has no webhook at all — this poll (driven by the buyer's
      // own top-up page) is the ONLY way a deposit gets credited. Same
      // observed-then-wait-out-the-confirm-window policy as escrow checkout
      // (lib/payment-routes.js's syncProviderPayment) — see
      // lib/payments/native-tron.js's doc comment for why.
      if (record.status !== 'paid' && record.provider === 'native_tron' && record.payAddress) {
        try {
          const seen = await payments.nativeTron.checkAddressForPayment(record.payAddress, record.amount);
          if (seen.found) {
            ...
            if (waited >= payments.nativeTron.confirmWindowMs()) {
              record = await creditDeposit(record);
            }
          }
        } catch (e) { /* fall through with current status */ }
  ...
  // Complete a sandbox top-up locally when there's no OxaPay key to actually
