  ...
  app.post('/api/wallet/topup', payLimit, async (req, res) => {
    try {
      ...
      if (native && native.configured()) {
        record.provider = payments.provider();
        record.method = record.provider;
      ...
      res.json({
        ...
        configured: oxapay.configured() || Object.values(payments.NATIVE_PROVIDERS).some(p => p.configured()),
      });
    } catch (err) {
